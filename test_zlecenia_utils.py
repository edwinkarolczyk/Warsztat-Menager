...
def test_przelicz_zapotrzebowanie_surowce():
    ...
    assert wynik["SR001"]["stan"] == pytest.approx(1.224)
    assert wynik["SR001"]["jednostka"] == "mb"
    assert wynik["SR002"]["stan"] == pytest.approx(1.575)
...
def test_sprawdz_magazyn_alerts_and_warnings(tmp_path):
    ...
    zap = {"SR001": {"stan": 115, "jednostka": "mb"}, "SR002": {"stan": 1, "jednostka": "mb"}}
    ok, alerts, warnings = sprawdz_magazyn(str(path), zap)
    assert ok is True
    assert alerts == ""
    assert "SR001" in warnings

    ok, alerts, _ = sprawdz_magazyn(str(path), {"SR002": {"stan": 100, "jednostka": "mb"}})
    assert ok is False
    assert "SR002" in alerts

    ok, alerts, _ = sprawdz_magazyn(str(path), {"SR999": {"stan": 1, "jednostka": "mb"}})
    assert ok is False
