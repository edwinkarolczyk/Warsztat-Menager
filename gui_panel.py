...
from logger import get_logger, log_akcja
    ...
    def panel_zlecenia(root, frame, login=None, rola=None):
        ...
        if isinstance(tab, (tk.Widget, ttk.Frame)):
            try:
                ...
                logger.exception("Failed to pack panel_zlecenia result")
        else:
            # awaryjnie pokaż etykietę, żeby nie było pusto
            ttk.Label(frame, text="Panel Zleceń – załadowano").pack(pady=12)

except Exception:
    logger.exception("Failed to import gui_zlecenia")

    def panel_zlecenia(root, frame, login=None, rola=None):
        for w in frame.winfo_children():
            w.destroy()
        ttk.Label(
            frame, text="Panel zleceń (fallback) – błąd importu gui_zlecenia"
        ).pack(pady=20)
...
try:
    ...
    logger.exception("Failed to import gui_narzedzia")
...
try:
    ...
    logger.exception("Failed to import gui_maszyny")
...
try:
    ...
    logger.exception("Failed to import gui_uzytkownicy")
...
try:
    ...
    logger.exception("Failed to import gui_profile")
    gui_profile = None

try:
    from ustawienia_systemu import panel_ustawien
except Exception:
    logger.exception("Failed to import ustawienia_systemu")
...
def uruchom_panel(root, login, rola):
    ...
    side = ttk.Frame(root, style="WM.Side.TFrame", width=220)
    side.pack(side="left", fill="y")
    main = ttk.Frame(root, style="WM.TFrame")
    main.pack(side="right", fill="both", expand=True)

    header = ttk.Frame(main, style="WM.TFrame")
    ...
    ttk.Label(header, text=f"{login} ({rola})", style="WM.Muted.TLabel").pack(
        side="right"
    )

    content = ttk.Frame(main, style="WM.Card.TFrame")
    content.pack(fill="both", expand=True, padx=12, pady=6)

    footer = ttk.Frame(main, style="WM.TFrame")
    footer.pack(fill="x", padx=12, pady=(6, 10))
    # lewa część: pasek zmiany (1/3 szerokości)
    shift_wrap = ttk.Frame(footer, style="WM.Card.TFrame")
    shift_wrap.pack(side="left")
    ...
    shift_info = ttk.Label(shift_wrap, text="", style="WM.Muted.TLabel")
    shift_info.pack(anchor="w", padx=8, pady=(0, 6))
    ...
    def _logout():
        ...
        try:
            ...
            logger.exception("presence.heartbeat logout failed")
        try:
            import gui_logowanie

            gui_logowanie.ekran_logowania(root)
        except Exception:
            logger.exception("Failed to open login screen")
            try:
                root.destroy()
            except Exception:
                logger.exception("Failed to destroy root window")

    btns = ttk.Frame(footer, style="WM.TFrame")
    btns.pack(side="right")
    ttk.Button(btns, text="Wyloguj", command=_logout, style="WM.Side.TButton").pack(
        side="right", padx=(6, 0)
    )
    ttk.Button(
    ...
    def draw_shift_bar():
        ...
        try:
            ...
            if done_w > 0:
                shift.create_rectangle(
                    0, 0, done_w, CANVAS_H, fill=done_color, outline=done_color
                )
            if done_w < CANVAS_W:
                shift.create_rectangle(
                    done_w,
                    ...
                    CANVAS_H,
                    fill=remain_color,
                    outline=remain_color,
                )

            shift_info.config(
                text=f"{s.strftime('%H:%M')} - {e.strftime('%H:%M')}    {percent}%"
            )
        except tk.TclError as e:
            logger.debug("Shift bar draw failed: %s", e)
            return
    ...
    def _on_shift_destroy(_e=None):
        if shift_job["id"]:
            try:
                ...
                logger.exception("Failed to cancel shift timer")
            shift_job["id"] = None
    ...
    def otworz_panel(funkcja, nazwa):
        ...
        try:
            ...
            logger.exception("Error opening panel %s", nazwa)
            ttk.Label(
                content, text=f"Błąd otwierania panelu: {e}", foreground="#e53935"
            ).pack(pady=20)
    ...
    def _open_profil():
        ...
        for w in content.winfo_children():
            try:
                ...
                logger.debug("Widget destroy failed: %s", e)
        try:
            ...
            logger.exception("Error opening profile panel")
            ttk.Label(
                content, text=f"Błąd otwierania panelu: {e}", foreground="#e53935"
            ).pack(pady=20)
    ...
    if str(rola).lower() in admin_roles:
        ...
        try:
            ...
            logger.exception("Failed to import panel_ustawien inside admin block")
    else:
        ttk.Button(
            side,
            text="Profil",
