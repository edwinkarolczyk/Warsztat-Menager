...
def _shift_rules() -> dict[str, dict[str, time]]:
    ...
    return {
        RANO: {"shift_start": rano_start, "shift_end": rano_end, "early_from": _move_time(rano_start, timedelta(hours=-1)), "auto_until": _move_time(rano_end, timedelta(hours=-2))},
        POPO: {"shift_start": popo_start, "shift_end": popo_end, "early_from": _move_time(popo_start, timedelta(hours=-1)), "auto_until": _move_time(popo_end, timedelta(hours=-2))},
    }
...
def _planned_slot_for_day(login: str, day: date) -> str | None:
    ...
    if day.weekday() not in workdays:
        return None
    mode = str(profile.get("tryb_zmian") or profile.get("shift_mode") or profile.get("zmiana") or "111").strip()
    return POPO if mode.startswith("2") else RANO
...
def _matching_record(slot_map: dict, login: str) -> tuple[str, dict] | tuple[None, None]:
    ...
    if user_key:
        ...
        for storage_key, candidate in slot_map.items():
            if isinstance(candidate, dict) and str(candidate.get("user_id") or "").strip().casefold() == user_key:
...
def _record(date_ymd: str, slot: str, login: str, *, create: bool = False) -> tuple[dict, dict, dict]:
    ...
    if not isinstance(rec, dict):
        ...
        if create:
            ...
            storage_key = str(uid or login or "").strip() or str(login or "").strip().casefold()
...
def absence_conflict(date_ymd: str, login: str) -> dict:
    """Zwróć aktywną nieobecność z ewidencji obecności/leaves dla wskazanego dnia."""
    reasons: list[str] = []
    reason_slot = ""
    doc = _read(data_path(), {})
    ...
    if isinstance(day, dict):
        for slot in (RANO, POPO):
            ...
            if not isinstance(slot_map, dict):
                continue
            _key, rec = _matching_record(slot_map, login)
            if not isinstance(rec, dict):
                continue
    ...
    for value in [*reasons, *leave_types]:
        normalized = "UR" if value == "URLOP" else value
...
def _audit(*, action: str, login: str, date_ymd: str, slot: str, actor: str, before: dict, after: dict, note: str = "") -> None:
    ...
    if not isinstance(rows, list):
        rows = []
    rows.append({"ts": _now_iso(), "action": action, "user_id": user_id_for(login), "login_snapshot": str(login or ""), "date": date_ymd, "slot": slot, "actor": str(actor or ""), "before": before, "after": after, "note": str(note or "").strip()})
...
def classify_login(slot: str, moment: datetime, *, saturday: bool = False) -> str:
    ...
    rules = _shift_rules().get(slot) or _shift_rules()[RANO]
    t = moment.time().replace(second=0, microsecond=0)
    return STATUS_PRESENT if rules["early_from"] <= t <= rules["auto_until"] else STATUS_PENDING_LATE
...
def mark_login(date_ymd: str, slot: str, login: str, ts_iso: str) -> None:
    ...
    before = dict(rec)
    first = str(rec.get("first_login_ts") or rec.get("logged_ts") or "").strip() or str(ts_iso or moment.isoformat(timespec="seconds"))
    ...
    if status == STATUS_PRESENT:
        rec.update({"status": STATUS_PRESENT, "day_value": float(rec.get("day_value") or 1.0), "confirmed": True, "approval_required": False})
        rec.setdefault("confirmed_by", "AUTO"); rec.setdefault("confirmed_ts", first); rec["source"] = rec.get("source") or "auto_login"
    elif status == STATUS_SATURDAY:
        rec.update({"status": STATUS_SATURDAY, "day_value": 0.0, "confirmed": False, "approval_required": True, "source": rec.get("source") or "auto_login"})
        rec.setdefault("overtime", {})
        if isinstance(rec["overtime"], dict):
            rec["overtime"].update({"type": "sobota", "day_value": float(rec["overtime"].get("day_value") or 1.0), "hours": rec["overtime"].get("hours"), "status": rec["overtime"].get("status") or "pending", "source": rec["overtime"].get("source") or "auto_login"})
    elif rec.get("status") != STATUS_PRESENT or not rec.get("confirmed"):
        rec.update({"status": STATUS_PENDING_LATE, "day_value": float(rec.get("day_value") or 0.0), "confirmed": False, "approval_required": True, "source": rec.get("source") or "auto_login"})
    _write(data_path(), doc)
    if before != rec:
        _audit(action="login", login=login_n, date_ymd=date_ymd, slot=resolved_slot, actor="AUTO", before=before, after=dict(rec))
...
def confirm_login(date_ymd: str, slot: str, login: str, bryg_login: str, ts_iso: str) -> None:
    ...
    before = dict(rec)
    rec.update({"planned": True, "status": STATUS_PRESENT, "day_value": 1.0, "confirmed": True, "approval_required": False, "confirmed_by": str(bryg_login or ""), "confirmed_ts": str(ts_iso or _now_iso()), "source": "foreman", "user_id": rec.get("user_id") or user_id_for(login_n)})
    rec.setdefault("login_snapshot", login_n)
    _write(data_path(), doc)
    _audit(action="confirm_day", login=login_n, date_ymd=date_ymd, slot=slot, actor=bryg_login, before=before, after=dict(rec))
...
def set_reason(date_ymd: str, slot: str, login: str, bryg_login: str, reason: str, ts_iso: str) -> None:
    ...
    doc, _slot_map, rec = _record(date_ymd, slot, login_n, create=True)
    before = dict(rec)
    rec.update({"planned": True, "reason": r, "status": STATUS_EXCUSED, "day_value": 0.0, "confirmed": False, "approval_required": False, "confirmed_by": str(bryg_login or ""), "confirmed_ts": str(ts_iso or _now_iso()), "source": "foreman", "user_id": rec.get("user_id") or user_id_for(login_n)})
    rec.setdefault("login_snapshot", login_n)
    _write(data_path(), doc)
    _audit(action="absence", login=login_n, date_ymd=date_ymd, slot=slot, actor=bryg_login, before=before, after=dict(rec), note=r)


def status_for(date_ymd: str, slot: str, login: str, shift_start: datetime, now: datetime, grace_hours: int = 4) -> str:
...
def set_manual_day(date_ymd: str, slot: str, login: str, value: float, actor: str, note: str = "", *, replace_absence: bool = False) -> dict:
    value = float(value)
    ...
    if conflict.get("has_conflict"):
        if conflict_slot and conflict_slot != slot:
            raise ValueError(f"Nieobecność jest zapisana na zmianie {conflict_slot}. Korektę wykonaj dla tej samej zmiany.")
        if not replace_absence:
            labels = ", ".join(conflict.get("reasons") or []) or "nieobecność"
            raise ValueError(f"Dzień ma wpis {labels}. Najpierw potwierdź zastąpienie nieobecności korektą.")
    doc, _slot_map, rec = _record(date_ymd, slot, login_n, create=True)
    before = dict(rec)
    rec.update({"planned": True, "reason": "" if replace_absence else str(rec.get("reason") or ""), "status": STATUS_PRESENT if value > 0 else STATUS_MISSING, "day_value": value, "confirmed": value > 0, "approval_required": False, "confirmed_by": str(actor or ""), "confirmed_ts": _now_iso(), "source": "foreman_manual", "manual_note": str(note or "").strip(), "user_id": rec.get("user_id") or user_id_for(login_n)})
    rec.setdefault("login_snapshot", login_n)
    _write(data_path(), doc)
    if replace_absence:
        try:
            ...
            rec.clear(); rec.update(before); _write(data_path(), doc)
            raise
    ...
    _audit(action=action, login=login_n, date_ymd=date_ymd, slot=slot, actor=actor, before=before, after=dict(rec), note=audit_note)
    return dict(rec)


def set_overtime(date_ymd: str, slot: str, login: str, hours: float, actor: str, *, overtime_type: str = "zwykle", day_value: float | None = None, note: str = "") -> dict:
    hours = max(0.0, float(hours))
    login_n = str(login or "").strip().casefold()
    doc, _slot_map, rec = _record(date_ymd, slot, login_n, create=True)
    before = dict(rec)
    overtime = dict(rec.get("overtime") or {})
    overtime.update({"type": str(overtime_type or "zwykle").strip().casefold(), "hours": hours, "status": "confirmed", "source": "foreman", "confirmed_by": str(actor or ""), "confirmed_ts": _now_iso(), "note": str(note or "").strip()})
    ...
    rec.setdefault("login_snapshot", login_n)
    _write(data_path(), doc)
    _audit(action="overtime", login=login_n, date_ymd=date_ymd, slot=slot, actor=actor, before=before, after=dict(rec), note=note)
    return dict(rec)


def _decision_due(day_obj: date, slot: str, now: datetime) -> bool:
    if day_obj < now.date(): return True
    if day_obj > now.date(): return False
    rules = _shift_rules().get(slot) or _shift_rules()[RANO]
    return now.time().replace(second=0, microsecond=0) > rules["auto_until"]


def _actual_month_records(login: str, year: int, month: int) -> list[dict]:
    login_n = str(login or "").strip().casefold(); doc = _read(data_path(), {}); prefix = f"{int(year):04d}-{int(month):02d}-"; rows=[]
    if not isinstance(doc, dict): return rows
    for day_text in sorted(doc):
        if not str(day_text).startswith(prefix): continue
        day_map = doc.get(day_text)
        if not isinstance(day_map, dict): continue
        for slot in (RANO, POPO):
            slot_map = day_map.get(slot)
            if not isinstance(slot_map, dict): continue
            _storage_key, rec = _matching_record(slot_map, login_n)
            if not isinstance(rec, dict): continue
            row=dict(rec); row.update({"date":str(day_text)[:10],"slot":slot,"login":login_n,"synthetic":False}); rows.append(row)
    return rows


def month_records(login: str, year: int, month: int, *, now: datetime | None = None) -> list[dict]:
    now=now or datetime.now(); login_n=str(login or "").strip().casefold(); actual=_actual_month_records(login_n,year,month); rows=[dict(r) for r in actual]
    actual_days={str(r.get("date") or "") for r in actual}; last=monthrange(int(year),int(month))[1]; start=date(int(year),int(month),1); end=min(date(int(year),int(month),last),now.date())
    current=start
    while current<=end:
        ...
        if day_text not in actual_days:
            ...
            if slot in VALID_SLOTS:
                due=_decision_due(current,slot,now); rows.append({"date":day_text,"slot":slot,"login":login_n,"user_id":user_id_for(login_n),"login_snapshot":login_n,"planned":True,"status":STATUS_MISSING if due else STATUS_PLANNED,"day_value":0.0,"confirmed":False,"approval_required":bool(due),"source":"schedule","reason":"","first_login_ts":"","logged_ts":"","synthetic":True})
        current+=timedelta(days=1)
    normalized=[]
    for row in rows:
        item=dict(row); day_obj=_parse_date(item.get("date")); slot=str(item.get("slot") or RANO); has_login=bool(str(item.get("first_login_ts") or item.get("logged_ts") or "").strip()); reason=str(item.get("reason") or "").strip(); status=str(item.get("status") or "")
        if day_obj and item.get("planned") and not has_login and not reason and status not in {STATUS_PRESENT,STATUS_EXCUSED} and _decision_due(day_obj,slot,now) and item.get("approval_required") is not False:
            item["status"]=STATUS_MISSING; item["approval_required"]=True
        elif not status and item.get("planned"): item["status"]=STATUS_PLANNED
        normalized.append(item)
    normalized.sort(key=lambda r:(str(r.get("date") or ""),0 if r.get("slot")==RANO else 1)); return normalized


def decision_records(login: str, year: int, month: int, *, now: datetime | None = None) -> list[dict]:
    now=now or datetime.now(); out=[]
    for row in month_records(login,year,month,now=now):
        if row.get("reason") or str(row.get("status") or "")==STATUS_EXCUSED or row.get("approval_required") is False: continue
        status=str(row.get("status") or "")
        if status not in {STATUS_PENDING_LATE,STATUS_SATURDAY,STATUS_MISSING}: continue
        item=dict(row); item["decision_label"]="Brak logowania" if status==STATUS_MISSING else ("Sobota — potwierdź" if status==STATUS_SATURDAY else "Logowanie po oknie — decyzja"); out.append(item)
    return out


def summary_for_month(login: str, year: int, month: int, *, now: datetime | None = None) -> dict[str,float]:
    out={"days":0.0,"saturday_days":0.0,"overtime_hours":0.0,"vacation":0.0,"l4":0.0,"missing":0.0,"pending":0.0,"manual":0.0}
    for rec in month_records(login,year,month,now=now):
        reason=str(rec.get("reason") or "").upper()
        if reason in {"UR","UŻ"}: out["vacation"]+=1.0
        elif reason=="L4": out["l4"]+=1.0
        status=str(rec.get("status") or "")
        if status==STATUS_PRESENT: out["days"]+=float(rec.get("day_value") or 0.0)
        elif status==STATUS_MISSING:
            out["missing"]+=1.0
            if rec.get("approval_required"): out["pending"]+=1.0
        elif status==STATUS_PENDING_LATE: out["pending"]+=1.0
        elif status==STATUS_SATURDAY and rec.get("approval_required"): out["pending"]+=1.0
        if str(rec.get("source") or "").startswith("foreman"): out["manual"]+=1.0
        ...
        if isinstance(overtime,dict) and overtime.get("status")=="confirmed":
            ...
            if str(overtime.get("type") or "").casefold()=="sobota": out["saturday_days"]+=float(overtime.get("day_value") or 1.0)
    return out


def audit_for_login(login: str, limit: int = 200) -> list[dict]:
    key=str(login or "").strip().casefold(); uid=str(user_id_for(login) or "").strip().casefold(); rows=_read(audit_path(),[])
    if not isinstance(rows,list): return []
    found=[dict(row) for row in rows if isinstance(row,dict) and ((uid and str(row.get("user_id") or "").strip().casefold()==uid) or str(row.get("login_snapshot") or "").strip().casefold()==key)]
    return found[-max(1,int(limit)):]


__all__ = [
    "RANO", "POPO", "SHIFT_RULES", "STATUS_PRESENT", "STATUS_PENDING_LATE", "STATUS_MISSING", "STATUS_EXCUSED", "STATUS_SATURDAY", "STATUS_PLANNED",
    "data_path", "audit_path", "mark_login", "confirm_login", "set_reason", "status_for", "set_manual_day", "set_overtime", "summary_for_month", "month_records", "decision_records", "audit_for_login", "classify_login", "user_id_for", "absence_conflict",
]
