...
def _load_tool_tasks(force: bool = False) -> dict[str, list[dict]]:
    ...
    try:
        ...
        logger.warning("Nie można odczytać %s: %s", TOOL_TASKS_PATH, e, exc_info=True)
        _TOOL_TASKS_CACHE = {}
        return {}
    ...
    try:
        ...
        logger.warning(
            "Nie można przetworzyć definicji zadań narzędzi: %s", e, exc_info=True
        )
        _TOOL_TASKS_CACHE = {}
        return {}
...
def get_tool_types_list(
    ...
    try:
        tasks = _load_tool_tasks(force=force)
    except Exception as e:  # pragma: no cover - defensive
        logger.warning("Nie można pobrać typów narzędzi: %s", e, exc_info=True)
        return []
    return [
...
def _find_type(
    ...
    coll = collection or _default_collection()
    try:
        types = _load_tool_tasks(force=force).get(coll, [])
    except Exception as e:  # pragma: no cover - defensive
        logger.warning("Nie można odnaleźć typu narzędzia: %s", e, exc_info=True)
        return None
    for t in types:
        if t.get("id") == type_id:
            return t
