...
def apply_theme(style: ttk.Style, name: str = "default") -> None:
    """Aplikuje motyw do ttk.Style."""

    if name not in THEMES:
        print(f"[WM-DBG][THEME] Motyw '{name}' nieznany, przełączam na 'default'")
        name = "default"
    c = THEMES[name]
    ...
    style.configure("H1.TLabel", font=("Segoe UI", 16, "bold"),
                    foreground=c["text"], background=c["panel"])
    style.configure("H2.TLabel", font=("Segoe UI", 13, "bold"),
                    foreground=c["text"], background=c["panel"])

    style.configure("TButton", background=c["card"], foreground=c["text"], padding=(10, 6))
    style.map("TButton",
    ...
    style.configure("TEntry", fieldbackground=c["entry_bg"], foreground=c["entry_fg"],
                    bordercolor=c["entry_bd"], insertcolor=c["text"], padding=6)
    style.configure("TCombobox", fieldbackground=c["entry_bg"], foreground=c["entry_fg"],
                    bordercolor=c["entry_bd"], arrowsize=14, padding=6)

    style.configure("TNotebook", background=c["panel"], borderwidth=0)
    style.configure("TNotebook.Tab", padding=(12, 6), background=c["panel"], foreground=c["muted"])
    ...
    style.configure("Treeview", background=c["card"], fieldbackground=c["card"],
                    foreground=c["text"], bordercolor=c["line"], rowheight=24)
...
def _resolve_widget_root(widget: tk.Misc | None) -> tk.Misc | None:
    ...
    try:
        ...
        logger.exception("Failed to resolve widget toplevel")
        return None
...
def apply_theme_safe(
    ...
    *,
    ...
    try:
        ...
        palette = THEMES[resolved_name]

        widgets: set[tk.Misc] = set()

        if isinstance(target, tk.Misc):
            root_widget = _resolve_widget_root(target)
            if root_widget is not None:
                widgets.add(root_widget)

        master_widget = getattr(style, "master", None)
        root_from_master = _resolve_widget_root(master_widget)
        if root_from_master is not None:
            widgets.add(root_from_master)
