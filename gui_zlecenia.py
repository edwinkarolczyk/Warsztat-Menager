...
try:
    ...
    try:
        ...
        logger.exception("Failed to import delete_zlecenie")
        _delete_zlecenie = None
except Exception:
    logger.exception("Failed to import zlecenia_logika")
    raise
...
def _bar10(percent: int) -> str:
    try:
        ...
        logger.exception("Invalid percent for _bar10")
        p = 0
    filled = p // 10
...
def panel_zlecenia(parent, root=None, app=None, notebook=None):
    ...
    # Pasek akcji
    actions = ttk.Frame(frame, style="WM.TFrame")
    actions.pack(fill="x", padx=12, pady=(0, 8))
    btn_nowe = ttk.Button(actions, text="Nowe zlecenie")
    btn_nowe.pack(side="left")
    btn_odsw = ttk.Button(actions, text="Odśwież")
    btn_odsw.pack(side="left", padx=6)
    btn_usun = ttk.Button(actions, text="Usuń")
    btn_usun.pack(side="left", padx=6)

    right = ttk.Frame(actions, style="WM.TFrame")
    right.pack(side="right")
    ttk.Label(right, text="Status:", style="WM.TLabel").pack(side="left", padx=(0, 6))
    cb_status = ttk.Combobox(
        right, state="readonly", values=["(wszystkie)"] + STATUSY, width=18
    )
    cb_status.current(0)
    cb_status.pack(side="left")
    ttk.Label(right, text="Szukaj:", style="WM.TLabel").pack(side="left", padx=(12, 6))
    ent_search = ttk.Entry(right, width=28)
    ent_search.pack(side="left")

    # Info bar
    info_bar = ttk.Frame(frame, style="WM.TFrame")
    info_bar.pack(fill="x", padx=12, pady=(0, 6))
    lbl_info = ttk.Label(
        info_bar, text="Panel Zleceń – odświeżono listę", style="WM.Muted.TLabel"
    )
    ...
    tree = ttk.Treeview(
        frame, columns=cols, show="headings", height=18, style="WM.Treeview"
    )
    tree.heading("id", text="ID")
    tree.column("id", width=110, anchor="center")
    tree.heading("zlec_wew", text="Tyczy nr")
    tree.column("zlec_wew", width=110, anchor="center")
    tree.heading("produkt", text="Produkt")
    tree.column("produkt", width=240, anchor="w")
    tree.heading("ilosc", text="Ilość")
    tree.column("ilosc", width=80, anchor="center")
    tree.heading("status", text="Status")
    tree.column("status", width=170, anchor="center")
    tree.heading("utworzono", text="Utworzono")
    tree.column("utworzono", width=180, anchor="center")
    tree.heading("postep", text="Postęp (10 kratek)")
    tree.column("postep", width=180, anchor="center")
    tree.pack(fill="both", expand=True, padx=12, pady=(0, 8))

    # Menu PPM + Delete
    menu = tk.Menu(tree, tearoff=False)
    menu.add_command(
        label="Usuń zlecenie", command=lambda: _usun_zlecenie(tree, lbl_info, _odswiez)
    )
    ...
    def _odswiez(*_):
        ...
        if not rows:
            tree.insert(
                "", "end", values=("— brak zleceń —", "", "", "", "", "", _bar10(0))
            )
        ...
        for z in rows:
            ...
            pct = (
                z.get("postep")
                if isinstance(z.get("postep"), int)
                else _STATUS_TO_PCT.get(stat, 0)
            )
    ...
    # Akcje
    btn_nowe.configure(
        command=lambda: _kreator_zlecenia(frame, lbl_info, root, _odswiez)
    )
...
def _kreator_zlecenia(parent: tk.Widget, lbl_info: ttk.Label, root, on_done) -> None:
    ...
    try:
        ...
        logger.exception("Failed to configure new order window background")
        try:
            win.configure(highlightthickness=0)
        except Exception:
            logger.exception("Fallback window configure failed")
    try:
        win.grab_set()
    except Exception:
        logger.exception("win.grab_set failed")
    ...
    try:
        ...
        logger.exception("list_produkty failed")
        produkty = []
    ...
    ttk.Label(frm, text="Tyczy się zlecenia nr (wew.)", style="WM.TLabel").grid(
        row=2, column=0, sticky="w", pady=(8, 0)
    )
    ent_ref = ttk.Entry(frm, width=18)
    ent_ref.grid(row=2, column=1, sticky="w", padx=(8, 0), pady=(8, 0))

    ttk.Label(frm, text="Uwagi", style="WM.TLabel").grid(
    ...
    try:
        txt.configure(
            ...
            highlightbackground=_DBG,
            highlightcolor=_DBG,
        )
    except Exception:
        logger.exception("Failed to configure text widget")
    ...
    def akcept():
        ...
        if not kod:
            messagebox.showwarning(
                "Brak produktu", "Wybierz produkt z listy.", parent=win
            )
            return
        try:
            ilosc = int(spn.get())
        except Exception:
            logger.exception("Invalid quantity in new order")
            messagebox.showwarning(
                "Błędna ilość", "Podaj prawidłową liczbę.", parent=win
            )
            return
...
def _edit_status_dialog(
    ...
    win = tk.Toplevel(parent)
    win.title(f"Status zlecenia {zlec_id}")
    _maybe_theme(win)
    try:
        win.configure(highlightthickness=0, highlightbackground=_DBG)
    except Exception:
        logger.exception("Failed to configure status window")
    try:
        win.grab_set()
    except Exception:
        logger.exception("win.grab_set failed for status window")
...
def _usun_zlecenie(tree: ttk.Treeview, lbl_info: ttk.Label, on_done):
    ...
    if _delete_zlecenie is None:
        messagebox.showinfo(
            "Usuń zlecenie",
            "Funkcja usuwania nieaktywna (brak delete_zlecenie w zlecenia_logika.py).",
        )
        return
    if not messagebox.askyesno(
        "Usuwanie zlecenia", f"Na pewno usunąć zlecenie {zid}?", icon="warning"
    ):
        return
    try:
        ...
        logger.exception("Usuwanie zlecenia failed")
