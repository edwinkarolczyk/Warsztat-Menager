...
class PlanistaPanel(ttk.Frame):
    ...
    def refresh(self):
        ...
        for o in ZL.list_zlecenia():
            ...
            self.tree.insert('', 'end', iid=oid, values=(oid,o.get('produkt',''),_fmt_qty(q),_fmt_qty(d),_fmt_qty(max(0,q-d)),_display_date(o.get('termin','')),o.get('status','')))
    def edit_term(self):
        o=self._selected()
        if not o: messagebox.showinfo('Planista','Wybierz zlecenie.',parent=self); return
        d=tk.Toplevel(self.root); d.title('Termin zlecenia'); d.transient(self.root); d.grab_set(); f=ttk.Frame(d,padding=12); f.pack(fill='both',expand=True); ttk.Label(f,text=f"Zlecenie: {o.get('id')} | Produkt: {o.get('produkt')}").grid(row=0,column=0,columnspan=3,sticky='w',pady=(0,8)); ttk.Label(f,text='Termin:').grid(row=1,column=0,sticky='w'); v=tk.StringVar(value=_display_date(o.get('termin')) or date.today().strftime('%d-%m-%y')); tk.Entry(f,textvariable=v,width=16,state='readonly',readonlybackground='#2e7d32',fg='white',relief='solid',bd=1,justify='center').grid(row=1,column=1,sticky='w',padx=(8,0)); ttk.Button(f,text='📅 Kalendarz',command=lambda:_open_date_calendar(d,v)).grid(row=1,column=2,sticky='w',padx=(8,0))
        def save():
            try: ZP.update_zlecenie(o['id'],termin=_iso_date(v.get()),kto=self.login or 'system')
    ...
    def edit_production(self):
        o=self._selected()
        if not o: messagebox.showinfo('Planista','Wybierz zlecenie.',parent=self); return
        d=tk.Toplevel(self.root); d.title('Parametry zlecenia'); d.transient(self.root); d.grab_set(); f=ttk.Frame(d,padding=12); f.pack(fill='both',expand=True)
        ttk.Label(f,text=f"Zlecenie {o.get('id')} — {o.get('produkt')}",font=('Arial',11,'bold')).grid(row=0,column=0,columnspan=3,sticky='w',pady=(0,8))
        ttk.Label(f,text='Ilość produktu:').grid(row=1,column=0,sticky='w'); qty=tk.StringVar(value=_fmt_qty(o.get('ilosc',0))); ttk.Entry(f,textvariable=qty,width=10).grid(row=1,column=1,sticky='w')
        ttk.Label(f,text='Rzaz na sztukę [mm]:').grid(row=2,column=0,sticky='w'); cut=tk.StringVar(value=_fmt_qty(o.get('rzaz_mm',2))); ttk.Entry(f,textvariable=cut,width=10).grid(row=2,column=1,sticky='w')
        ttk.Label(f,text='Półprodukt').grid(row=3,column=0,sticky='w',pady=(10,2)); ttk.Label(f,text='Wyliczono').grid(row=3,column=1,sticky='w'); ttk.Label(f,text='Do zlecenia').grid(row=3,column=2,sticky='w')
        vars={}
        for r,(code,rec) in enumerate((o.get('plan_polprodukty') or {}).items(),start=4):
        ...
        def save():
            try:
                qv=float(qty.get().replace(',','.')); cv=float(cut.get().replace(',','.')); ov={k:float(v.get().replace(',','.')) for k,v in vars.items()}
                if qv<0 or cv<0 or any(x<0 for x in ov.values()): raise ValueError('Ilość, rzaz i półprodukty nie mogą być ujemne.')
                ZP.update_zlecenie(o['id'],ilosc=qv,rzaz_mm=cv,korekty_polproduktow=ov,kto=self.login or 'system')
            except Exception as x: messagebox.showerror('Planista',str(x),parent=d); return
            d.destroy(); self.refresh()
    ...
    def report_done(self):
        ...
        def save():
            try: ZP.report_wykonano(o['id'],float(v.get().replace(',','.')),kto=self.login or 'system')
