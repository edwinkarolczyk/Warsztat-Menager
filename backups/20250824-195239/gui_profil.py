...
# Data: 2025-08-24
# Patch G+:
#  - Do okna szczegółów ZLECENIA (dla brygadzisty) dodano combobox z listą użytkowników
#    w polu „Przypisz do (login)”, z autouzupełnieniem i możliwością ręcznej edycji.
#  - Źródła listy loginów (w tej kolejności):
#      1) data/users.json  (lista obiektów z kluczem 'login' LUB lista stringów)
#      2) nazwy plików: data/zadania_*.json, data/zlecenia_*.json
#      3) nazwy plików avatarów avatars/*.png
#  - Pozostałe funkcje z Patch G: overrides statusów, przypisania zleceń, overdue, skan katalogów.
#
#  UWAGA: Nie ingeruje w Twoje formaty – wszystko „doklejone” bez ruszania logiki źródłowej.
...
# lista loginów do comboboxa (dla brygadzisty)
def _load_user_logins():
    logins = []
    ...
    if os.path.exists(ufile):
        try:
            data = _read_json(ufile)
            if isinstance(data, list):
                for item in data:
                    if isinstance(item, str):
                        logins.append(item)
                    elif isinstance(item, dict):
                        v = item.get("login") or item.get("name") or item.get("user")
                        if v: logins.append(str(v))
        except Exception:
            pass

    # 2) pliki zadania_*.json i zlecenia_*.json
    for pat in ("data/zadania_*.json", "data/zlecenia_*.json"):
        for path in glob.glob(pat):
            base = os.path.basename(path)
            name = base.split("_",1)[-1].replace(".json","")
    ...
    # unikaty + sort
    uniq = sorted({s.strip() for s in logins if s and isinstance(s, str)})
    return uniq

def _is_order_assigned_to(order, login, rola):
    # check direct assignment in file
    ...
    if oid:
        ...
        if str(amap.get(str(oid),"")).lower() == str(login).lower():
            return True
    # unassigned visible only to brygadzista
    if rola == "brygadzista":
        return True
    return False
...
# ---------- UI / SAVE ----------
def _save_status(login, task_id, new_status):
    _save_status_override(login, task_id, new_status)
...
def _show_task_details(root, frame, login, rola, task, all_tasks):
    ...
    # przypisanie zlecenia (tylko dla orderów i tylko brygadzista)
    assign_var = tk.StringVar(value="")
    if task.get("_kind")=="order" and rola=="brygadzista":
        frm = ttk.Frame(win); frm.pack(fill="x", padx=8, pady=6)
        ttk.Label(frm, text="Przypisz do (login):").pack(side="left")

        # lista loginów do comboboxa
        user_list = _load_user_logins()
        ent = ttk.Combobox(frm, textvariable=assign_var, values=user_list, state="normal")  # free text allowed
        ent.pack(side="left", padx=6)

        # aktualne przypisanie (z pliku lub overrides)
        cur = task.get("login") or ""
        from_map = _load_assign_map().get(str(task.get("zlecenie")))
        if from_map: cur = from_map
        assign_var.set(cur)
    ...
    def _save_and_close():
        # status override
        new_st = status_var.get()
        _save_status(login, task.get("id",""), new_st)
        task["status"] = new_st  # odśwież w pamięci

        # assignment (brygadzista + order)
        if task.get("_kind")=="order" and rola=="brygadzista":
...
def _build_tasks(frame, root, login, rola, tasks):
    ...
    tv = ttk.Treeview(container, columns=cols, show="headings", height=12)
    for c,w in zip(cols,(160,480,160,160)):
        tv.heading(c, text=c.capitalize()); tv.column(c, width=w, anchor="w")
        # można dodać sortowanie po kliknięciu nagłówka (opcjonalnie)
    ...
    tv.tag_configure("OVERDUE", foreground="#dc2626")  # czerwony dla po terminie

    today = _dt.now().date()

    def _tag(status, termin):
        s=(status or '').lower()
        if termin:
            d=_parse_date(termin)
            if d and d<today and s!="zrobione":
                return "OVERDUE"
    ...
    for z in tasks:
        tv.insert("", "end",
                  ...
                  tags=(_tag(z.get("status"), z.get("termin")),))
