...
# Wersja pliku: 1.3.4
# Data: 2025-08-24
# Patch F+: wsparcie katalogów narzędzi i zleceń + reguła dla nieprzypisanych zleceń
# - data/narzedzia/*.json → zadania przypisanego pracownika
# - data/zlecenia/*.json → zlecenia przypisane do loginu
# - NOWE: jeśli brak przypisania → pokazuj tylko brygadziście
# - scala także wszystkie poprzednie źródła (Patch D/E/F)
# UI: avatar 96px, statystyki, lista zadań, dwuklik = szczegóły/edycja, guzik Otwórz zlecenie.
    ...
    for key in ("login","operator","pracownik","przydzielono","assigned_to"):
        ...
        if val:
            ...
            if isinstance(val, list) and str(login).lower() in [str(v).lower() for v in val]:
                return True
    # jeśli nie ma żadnego przypisania → tylko dla brygadzisty
    if rola=="brygadzista":
        return True
    return False
...
def _build_tasks(frame, root, login, rola, tasks):
    ...
    for c,w in zip(cols,(140,480,140,160)):
