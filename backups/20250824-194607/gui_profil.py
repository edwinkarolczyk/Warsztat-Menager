...
# Wersja pliku: 1.4.0
# Data: 2025-08-24
# Patch G: trwałe zmiany w Profilu + przypisywanie zleceń przez brygadzistę
# - ZMIANA STATUSU działa dla KAŻDEGO wpisu (zadanie/zlecenie) dzięki 'overrides':
#     data/profil_overrides/status_<login>.json  → { "TASK_ID": "Nowe|W toku|Pilne|Zrobione", ... }
#   (nie nadpisujemy plików źródłowych)
# - PRZYPISANIE ZLECENIA (tylko brygadzista):
#     data/profil_overrides/assign_orders.json → { "ORDER_ID": "login" }
#   (po przypisaniu zlecenie zaczyna się wyświetlać danemu użytkownikowi)
# - Overdue (po terminie): jeżeli termin < dziś i status != Zrobione → tag OVERDUE (czerwony)
# - Obsługa katalogów: data/narzedzia/*.json, data/zlecenia/*.json + wszystkie wcześniejsze źródła (D/E/F).
# - UI: avatar 96px, statystyki, lista, dwuklik → szczegóły (status + przypisanie zlecenia jeżeli brygadzista).
...
def _show_task_details(root, frame, login, rola, task, all_tasks):
    ...
    if task.get("_kind")=="order" and rola=="brygadzista":
        ...
        ent = ttk.Combobox(frm, textvariable=assign_var, values=[], state="normal")  # allow free text
        ent.pack(side="left", padx=6)
        # pokaż aktualne przypisanie (z pliku lub overrides)
