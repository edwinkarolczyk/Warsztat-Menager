...
# Wersja pliku: 1.2.0
# Data: 2025-08-23
# Cel: Pełny panel profilu (avatar + login/rola + aktualna zmiana + statystyki zadań + lista zadań z kolorami)
# Zgodność: gui_panel.py 1.6.16 (sygnatura wywołania: uruchom_panel(root, frame, login, rola))
# Uwaga: Bez zależności zewnętrznych; korzysta z tk.PhotoImage dla PNG i fallback Canvas dla inicjałów.
# ⏹ KONIEC KODU
...
def _shift_bounds_label():
    """Zwraca tekst 'I/II/III (HH:MM–HH:MM)' na podstawie gui_panel._shift_bounds."""
    try:
        import gui_panel
...
def _load_avatar(parent, login):
    # Spróbuj wczytać PNG avatars/<login>.png
    if login:
        ...
        if os.path.exists(p):
            try:
                ...
                pass
    # Fallback – inicjały
    c = tk.Canvas(parent, width=64, height=64, highlightthickness=0, bg="")
    c.create_oval(2,2,62,62, fill="#1f2937", outline="#93a3af")  # tło/obrys
...
def _stats_from_tasks(tasks):
    ...
    done_cnt = sum(1 for t in tasks if (t.get("status",""))
                   and (t.get("status"," ").strip().lower() in ("zrobione","done","closed","zamknięte")))
...
def uruchom_panel(root, frame, login=None, rola=None):
    ...
    # Tasks: statystyki + lista
    tasks = _read_tasks(login)
    _build_stats(frame, tasks)
    _build_tasks(frame, root, login, rola)
    return frame

# Alias zgodny z Twoim gui_panel.py 1.6.16
