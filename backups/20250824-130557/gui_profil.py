# Plik: gui_profil.py
# Wersja pliku: 1.3.1
# Patch D (23.08.2025): konsolidacja zadań z wielu źródeł
#  - do _read_tasks(login) dodano merge z:
#      * data/zadania_<login>.json  (osobiste)
#      * data/zadania.json          (centralne)
#      * data/zadania_narzedzia.json (z modułu Narzędzia)
#      * data/zadania_zlecenia.json  (z modułu Zlecenia)
#  - filtrowanie po polu "login" (jeśli plik centralny), bez zmian UI.
#
# UWAGA: Podmień tylko, jeśli masz już wersję 1.3.0. To jest drop-in.
#       Reszta pliku identyczna jak 1.3.0, zmieniona wyłącznie funkcja _read_tasks().
import os, json
...
def _read_tasks(login):
    ...
    # 4) ze Zleceń
    p_orders = os.path.join("data", "zadania_zlecenia.json")
    if os.path.exists(p_orders):
        all_tasks.extend([z for z in read_json(p_orders) if str(z.get("login")) == str(login)])

    return all_tasks

# --- poniżej pozostaje bez zmian w stosunku do 1.3.0 ---
