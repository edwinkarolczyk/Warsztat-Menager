...
# Plik: gui_profil.py
# Wersja pliku: 1.6.3 (Patch H2c – FULL)
# Zasady widoczności:
#  - uzytkownik: widzi TYLKO swoje (zlecenia/narzędzia przypisane w źródłach lub przez override)
#  - brygadzista: widzi WSZYSTKO
# Override'y:
#  - data/profil_overrides/assign_orders.json  → przypisania zleceń (klucz = numer zlecenia)
#  - data/profil_overrides/assign_tools.json   → przypisania zadań narzędzi (klucz = ID zadania: NARZ-<nr>-<idx>)
#  - data/profil_overrides/status_<login>.json → statusy zadań
#  Nie modyfikujemy oryginalnych plików w data/*.
...
def uruchom_panel(root, frame, login=None, rola=None):
    ...
    try:
        apply_theme(root); apply_theme(frame)
    except Exception:
        pass
