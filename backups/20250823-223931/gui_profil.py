...
# Wersja pliku: 1.3.0
# Data: 2025-08-23
# Patch C: Pełny Profil + podgląd/edycja zadań + zlecenia
# - Avatar 96x96
# - Dwuklik: okno szczegółów zadania (opis, status, termin, powiązane zlecenie)
# - Edycja statusu (Combobox + zapis do JSON)
# - Łączenie zadań ze zleceniami (guzik 'Otwórz zlecenie')
# - Statystyki i kolory zadań
...
def _read_tasks(login):
    ...
    if os.path.exists(p1):
        try:
            with open(p1, "r", encoding="utf-8") as f: return json.load(f)
        except Exception: pass
    p2 = os.path.join("data", "zadania.json")
    if os.path.exists(p2):
        try:
            with open(p2, "r", encoding="utf-8") as f: data=json.load(f)
...
def _save_tasks(login, tasks):
    ...
    try:
        ...
        print(f"[WM-DBG][PROFIL] zapisano zmiany do {p1}")
...
def _open_zlecenie(root, frame, login, rola, zlec_id):
    try:
        ...
        if fn:
            return fn(root, frame, login, rola) # uproszczone – nie zawsze podświetli ID
    except Exception as e:
...
def _show_task_details(root, frame, login, rola, task, all_tasks):
    ...
    win.title(f"Zadanie {task.get('id','')} – szczegóły")
    _safe_theme(win)
...
def _build_tasks(frame, root, login, rola, tasks):
    ...
    def on_dbl(event):
        ...
        if sel:
            idx=tv.index(sel[0])
            _show_task_details(root, frame, login, rola, tasks[idx], tasks)
