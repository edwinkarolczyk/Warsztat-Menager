...
def ekran_logowania(root=None, on_login=None, update_available=False):
    ...
    if os.path.exists(logo_path):
        try:
            ...
            lbl_logo = tk.Label(
                top, image=logo_img, bg=root["bg"] if "bg" in root.keys() else "#0f1113"
            )
            lbl_logo.image = logo_img  # pin referencji
            lbl_logo.pack()
        except Exception:
            logger.exception("Nie można załadować logo")
    ...
    try:
        ...
        logger.exception("today_summary error")
    ...
    )  # trochę wyżej niż idealne 0.5, by było miejsce na pasek
    ...
    ttk.Button(box, text="Zaloguj", command=logowanie, style="WM.Side.TButton").pack(
        pady=16
    )
    ...
    CANVAS_H = 18
    shift = tk.Canvas(
        wrap, width=CANVAS_W, height=CANVAS_H, highlightthickness=0, bd=0, bg="#1b1f24"
    )
    ...
    def draw_login_shift():
        ...
        try:
            ...
            logger.debug("draw_login_shift canvas error: %s", e)
            return
    ...
    def _on_destroy(_e=None):
        if shift_job["id"]:
            try:
                ...
                logger.exception("Failed to cancel login shift timer")
            shift_job["id"] = None
    ...
    ttk.Button(
        bottom, text="Zamknij program", command=zamknij, style="WM.Side.TButton"
    ).pack()
    # stopka
    ttk.Label(
        root, text="Warsztat Menager – Wersja 1.4.12.1", style="WM.Muted.TLabel"
    ...
    try:
        ...
        logger.exception("Git fetch status failed")
